...
int main(int argc, char const *argv[]) {
  ...
  float elapsed_time_mem_alloc, elapsed_time_mem_t_in, elapsed_time_mem_t_f,
      elapsed_time_mem_t_out, elapsed_time_kernel;
  cudaEvent_t beg, end;
  cudaEventCreate(&beg);
  cudaEventCreate(&end);
  // ---------------------------------------------------------- //
  ...
  std::cout << "Loaded image with Width: " << n_cols
            << " and Height: " << n_rows << "\n";
  std::cout << "\n";
  ...
  std::cout << "Time for GPU memory allocation (seconds): "
            << elapsed_time_mem_alloc << "\n";
  std::cout << "\n";
  ...
  err = cudaMemcpy(d_N, N, new_size * new_size * sizeof(float),
  ...
  std::cout << "Time for input data transfer (seconds): "
            << elapsed_time_mem_t_in << "\n";
  std::cout << "\n";
  ...
  float *F;
  err =
      cudaMallocHost((void **)&F, (2 * FILTER_RADIUS + 1) *
  ...
  std::cout << "Time for filter data transfer (seconds): "
            << elapsed_time_mem_t_f << "\n";
  std::cout << "\n";
  ...
  dim3 dim_grid(ceil(new_size / (float)(OUTPUT_TILE_DIM)),
                ceil(new_size / (float)(OUTPUT_TILE_DIM)), 1);
  // Warmup
  for (int i = 0; i < 10; i++)
    gpu_conv2d_tiled_kernel<<<dim_grid, dim_block>>>(d_N, d_P, new_size,
                                                     new_size);
  ...
  err = cudaMemcpy(P, d_P, new_size * new_size * sizeof(float),
  ...
  std::cout << "Time for output data transfer (seconds): "
            << elapsed_time_mem_t_out << "\n";
  std::cout << "\n";
  ...
  std::cout << "--------------------- \n";
  std::cout << "Time (total): "
            << elapsed_time_kernel + elapsed_time_mem_alloc +
                   elapsed_time_mem_t_in + elapsed_time_mem_t_f +
                   elapsed_time_mem_t_out
            << "\n";
  std::cout << "FPS (total): "
            << 1 / (elapsed_time_kernel + elapsed_time_mem_alloc +
                    elapsed_time_mem_t_in + elapsed_time_mem_t_f +
                    elapsed_time_mem_t_out)
            << "\n";
  std::cout << "\n";
  ...
  std::cout << "GFLOPS (kernel): "
            << 2 * new_size * new_size * (2 * FILTER_RADIUS + 1) *
                   (2 * FILTER_RADIUS + 1) * 1e-9 / elapsed_time_kernel
            << "\n";
  std::cout << "------------------------------------ \n";
  std::cout << "\n";
  ...
  for (int i = 0; i < n_rows; i++) {
    for (int j = 0; j < n_cols; j++)
      ucharData[i * n_cols + j] = static_cast<unsigned char>(
          std::min(std::max(P[i * new_size + j], 0.0f), 1.0f) * 255.0f);
  }

  // Write the output image to a file
  stbi_write_png("./gpu_pinnedTiled_benchmark_output_img.png",
                 n_cols, n_rows, 1, ucharData.data(), n_cols);

  // ----------------------------------------------------------------- //
  // ------------------ Saving benchmark results --------------------- //
  // ----------------------------------------------------------------- //
  // Opening File
  const std::string filename = "./gpu_pinnedTiled.txt";
  ...
  file << "Time (total): "
       << elapsed_time_kernel + elapsed_time_mem_alloc + elapsed_time_mem_t_in +
              elapsed_time_mem_t_f + elapsed_time_mem_t_out
       << "\n";
  file << "FPS (total): "
       << 1 / (elapsed_time_kernel + elapsed_time_mem_alloc +
               elapsed_time_mem_t_in + elapsed_time_mem_t_f +
