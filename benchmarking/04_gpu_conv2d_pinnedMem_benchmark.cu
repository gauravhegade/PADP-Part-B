...
int main(int argc, char const *argv[]) {
  ...
  float *d_F;
  err = cudaMalloc((void **)&d_F, (2 * FILTER_RADIUS + 1) *
  ...
  err = cudaMemcpy(
      d_F, F, (2 * FILTER_RADIUS + 1) * (2 * FILTER_RADIUS + 1) * sizeof(float),
      cudaMemcpyHostToDevice);
  cuda_check(err);
  ...
  // Warmup
  for (int i = 0; i < 10; i++)
    gpu_conv2d_kernel<<<dim_grid, dim_block>>>(d_N, d_F, d_P, new_size,
                                               new_size);
  ...
  stbi_write_png("./gpu_pinned_benchmark_output_img.png", n_cols,
                 n_rows, 1, ucharData.data(), n_cols);

  // ----------------------------------------------------------------- //
  // ------------------ Saving benchmark results --------------------- //
  // ----------------------------------------------------------------- //
  // Opening File
  const std::string filename = "./gpu_pinned.txt";
